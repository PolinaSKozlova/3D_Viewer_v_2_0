...
Model ObjParser::ParseData(const std::string& filename) {
  ...
  if (!ifs.is_open()) { throw runtime_error("Could not open file"); }
  ...
  while (getline(ifs, line)) {
    ...
    if (type == "v") {
      double x{}, y{}, z{};
      if (!(iss >> x >> y >> z)) { throw runtime_error("Invalid vertex"); }
  ...
  if (model.vertices.empty() ||
      model.faces.empty()) { throw runtime_error("No vertices"); }
  return model;
}
...
Model ObjParser::CalculateScaler(Model& model) {
  ...
  for (size_t i = 0; i < model.vertices.size(); i += 3) {
    if (model.vertices[i] > x_max) { x_max = model.vertices[i]; }
    if (model.vertices[i+1] > y_max) { y_max = model.vertices[i+1]; }
    if (model.vertices[i+2] > z_max) { z_max = model.vertices[i+2]; }
  }
...
std::ostream& operator<<(std::ostream& os, const Model& model) {
  os << "Vertices:\n";
  for (size_t i = 0; i < model.vertices.size(); i += 3) {
    os << "(" << model.vertices[i] << ", " << model.vertices[i + 1] << ", " << model.vertices[i + 2] << ")\n";
  }
  os << "Faces:\n";
  for (size_t i = 0; i < model.faces.size(); i += 3) {
    os << "(" << model.faces[i] << ", " << model.faces[i + 1] << ", " << model.faces[i + 2] << ")\n";
  }
